  ...
  return new Promise((resolve, reject) => {
    ...
    crypto.scrypt(
      ...
      (err, derived) => {
        ...
        resolve(
          `scrypt$16384$8$1$${salt}$${derived.toString('base64url')}`
        );
      }
    );
  });
...
async function route(req, res, url) {
  ...
  if (
    ...
    if (
      ...
      return json(res, 429, {
        error:
          'Majaribio mengi ya login. Subiri dakika chache.'
      });
    }
    ...
    if (
      ...
      return json(res, 400, {
        error:
          'Email/Reg No na password vinahitajika.'
      });
    }
  ...
  if (
    ...
    if (
      ...
      return json(res, 400, {
        error:
          'Password mpya iwe na characters 8 hadi 128.'
      });
    }
  ...
  if (
    ...
    const {
      ...
      .from('users')
      .select(
        'id,auth_user_id,full_name,email,reg_no,role,is_password_changed,created_at'
      )
      .order('created_at', {
        ascending: false
      });
  ...
  if (
    ...
    if (
      ...
      return json(res, 400, {
        error:
          'Jina, email na Reg No vinahitajika.'
      });
    }
  ...
  if (
    ...
    if (target.role === 'admin') {
      return json(res, 403, {
        error:
          'Admin account haiwezi kufutwa hapa.'
      });
    }
  ...
  if (
    ...
    const {
      ...
      .from('messages')
      .select(
        'id,sender_name,sender_auth_id,message_text,created_at'
      )
      .order('created_at', {
        ascending: true
      })
  ...
  if (
    ...
    if (
      ...
      return json(res, 429, {
        error:
          'Umetuma ujumbe mwingi. Subiri kidogo.'
      });
    }
...
function serveStatic(
  ...
  res.writeHead(200, {
    ...
    'Content-Security-Policy':
      "default-src 'self'; script-src 'self' 'unsafe-inline' https://meet.jit.si; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://meet.jit.si wss://meet.jit.si; frame-src 'self' https://meet.jit.si; object-src 'none'; base-uri 'self'; form-action 'self'",
    'Permissions-Policy':
...
const server =
  http.createServer(
    ...
    const status =
      /Unauthorized|expired|not found/i.test(message)
        ? 401
        : 500;
